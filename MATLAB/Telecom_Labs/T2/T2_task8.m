...
[audio_samples,Fs] = audioread("rec2.wav"); %array containing samples of the recorded audio
...
bit_stream = zeros([1,number_of_bits]); %array which is used to store the binary representation of the recorded audio
...
for k=1:21
Amplitude = Amplitudes(k); %Amplitude in decibels
sampling_rate = Fs*qbits*samples_per_bit; %The sampling rate used to represent the analog singal in MATLAB
...
decoded_bit_stream_error_corrected = zeros([1,number_of_bits]); %The array to store the decoded bit stream after error−correction
for i = 1:number_of_bits %This loop does the task of correcting erros by looking at the most frequent bit from the three bits
for j = 1:3
...
BER = sum(bit_stream~=decoded_bit_stream_error_corrected)/length(bit_stream)
disp(Amplitude)
arr_BER(end+1)=BER;
end

figure(1)
plot(Amplitudes,arr_BER)
title('Amplitude vs BER')
xlabel('Amplitude(dB)')
ylabel('BER')
ylabel('Amplitude')
